...
class MuayeneDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['recete_list'] = Recete.objects.filter(muayene__pk = muayene_pk).order_by('-pk')[:5]
        context['rapor_list'] = Rapor.objects.filter(muayene__pk = muayene_pk).order_by('-pk')[:5]
        context['lab_list'] = LaboratuvarIstek.objects.filter(muayene__pk = muayene_pk).order_by('-pk')[:5]
        context['file_list'] = MuayeneRelatedFile.objects.filter(muayene__pk = muayene_pk).order_by('-pk')[:5]
